...
class VDNAgent(object):
    ...
    def __init__(self,
        ...
        self.params += list(self.qmixer_model.parameters())
    ...
    def learn(self, state_batch, actions_batch, reward_batch, terminated_batch,
        ...
        if self.qmixer_model is not None:
            global_qs = self.qmixer_model(target_local_qs)
            target_global_qs = self.target_qmixer_model(target_local_max_qs)

        # Calculate 1-step Q-Learning targets
        target = reward_batch + self.gamma * (
            1 - terminated_batch) * target_global_qs
        #  Td-error
        td_error = target.detach() - global_qs
