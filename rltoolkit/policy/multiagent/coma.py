...
class COMA(Algorithm):
    ...
    def _get_critic_output(self, batch):
        ...
        critic_inputs = critic_inputs.reshape(
    ...
    def _get_actor_output(self, batch, epsilon):
        ...
        for tr_id in range(tr_num):
            inputs = actor_inputs[:, tr_id]
            # shape (ep_num, n_agents, actor_input_dim)
            inputs = inputs.reshape((-1, inputs.shape[-1]))
            # shape (-1, actor_input_dim)
        ...
        action_prob = torch.stack(action_prob, dim=1).to(self.device)
        # shape: (ep_num, tr_num, n_agents, n_actions)
    ...
    def _cal_td_target(self, batch, q_targets):  # compute TD(lambda)
        ...
        isover = (1 - batch['isover'].float()).repeat(1, 1, self.n_agents).to(
            self.device)
        # used for setting last transition's q_target to 0
        ...
        n_step_return = torch.zeros(
            (ep_num, tr_num, self.n_agents, tr_num)).to(self.device)
        for tr_id in range(tr_num - 1, -1, -1):
            n_step_return[:, tr_id, :,
                          0] = (r[:, tr_id] + self.gamma * q_targets[:, tr_id]
        ...
        lambda_return = torch.zeros(
            (ep_num, tr_num, self.n_agents)).to(self.device)
        for tr_id in range(tr_num):
            returns = torch.zeros((ep_num, self.n_agents)).to(self.device)
            for n in range(1, tr_num - tr_id):
                returns += pow(self.td_lambda,
                               n - 1) * n_step_return[:, tr_id, :, n - 1]
            lambda_return[:, tr_id] = (1 - self.td_lambda) * returns + pow(
                self.td_lambda, tr_num - tr_id -
                1) * n_step_return[:, tr_id, :, tr_num - tr_id - 1]
    ...
    def _critic_learn(self, batch):
        ...
        loss = pow(masked_td_error, 2).sum() / mask.sum()
        # mask.sum: avail transition num
