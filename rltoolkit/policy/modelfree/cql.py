...
class CQL(Algorithm):
    ...
    def __init__(self,
        ...
        Args:
            ...
            lagrange_thresh (float): the value of tau, corresponds to the CQL(lagrange) version, \
                suggest 10.0 in mujoco and 5.0 in Franka kitchen or Adroit domains
    ...
    def predict(self, obs):
        """Define the predicting process, e.g,.
    ...
    def sample(self, obs):
        ...
        This function returns an action with noise to perform exploration.
        """
    ...
    def learn(self, obs, action, reward, next_obs, terminal):
        """Define the loss function and create an optimizer to minize the
        loss."""
    ...
    def _critic_learn(self, obs, action, reward, next_obs, terminal):
        ...
        random_actions_tensor = torch.FloatTensor(
            cur_q2.shape[0] * self.num_random,
            action.shape[-1]).uniform_(-1, 1).to(self.device)
    ...
    def _actor_learn(self, obs, action):
        ...
        if self._current_steps < self.policy_eval_start:
            """For the initial few epochs, try doing behaivoral cloning, if
            needed conventionally, there's not much difference in performance
            with having 20k gradient steps here, or not having it."""
