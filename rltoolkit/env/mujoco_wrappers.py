'''
Author: jianzhnie
Date: 2022-09-01 15:32:09
LastEditors: jianzhnie
LastEditTime: 2022-09-01 15:32:26
Description:
...
class MonitorEnv(gym.Wrapper):
    """Env wrapper that keeps tracks of total raw episode rewards, length of
    raw episode rewards for evaluation."""
...
class RunningMeanStd(object):
    """Calculating running mean and variance https://en.wikipedia.org/wiki/Algo
    rithms_for_calculating_variance#Parallel_algorithm."""
...
class VecNormalizeEnv(gym.Wrapper):
    """Env wrapper that normalize reward, observation based on running mean
    return and running mean variance."""
...
def get_ob_rms(env):
    """get ob_rms value from current env, if current env does not wrap
    VecNormalizeEnv, None will be returned.
...
def wrap_rms(env, gamma, test=False, ob_rms=None):
    """Wrap original Mujoco environment with wrapper envs to provide
    normalization using rms and extra functionality, rewards information are
    stored in info['episode']. This is the wrapper for single agent.
