...
# modified from https://github.com/AI4Finance-Foundation/ElegantR
...
class ReplayBuffer_isaacgym:  # for off-policy
    ...
    def __init__(self,
        ...
        self.buf_action = torch.empty((self.max_capacity, int(action_dim)),
                                      dtype=torch.float32,
                                      device=self.device)
        self.buf_reward = torch.empty((self.max_capacity, 1),
                                      dtype=torch.float32,
                                      device=self.device)
        self.buf_done = torch.empty((self.max_capacity, 1),
                                    dtype=torch.float32,
                                    device=self.device)

        buf_state_size = (self.max_capacity, state_dim) if isinstance(
            state_dim, int) else (max_capacity, *state_dim)
        self.buf_state = torch.empty(
            buf_state_size, dtype=torch.float32, device=self.device)
        self.buf_next_state = torch.empty(
            buf_state_size, dtype=torch.float32, device=self.device)

        self.if_use_per = if_use_per
        self.per_tree = BinarySearchTree(
            self.max_capacity) if self.if_use_per else None

    def update_buffer(self, trajectory: Tuple[Tensor, Tensor, Tensor, Tensor,
                                              Tensor]):
        if self.if_use_per:
        ...
        if self.if_use_per:
            self.per_tree.update_ids(
                data_ids=np.arange(self.next_p, p) % self.max_capacity)

        if p > self.max_capacity:
            self.buf_state[self.next_p:self.
                           max_capacity] = states[:self.max_capacity -
                                                  self.next_p]
            self.buf_action[self.next_p:self.
                            max_capacity] = actions[:self.max_capacity -
                                                    self.next_p]
            self.buf_reward[self.next_p:self.
                            max_capacity] = rewards[:self.max_capacity -
                                                    self.next_p]
            self.buf_next_state[self.next_p:self.
                                max_capacity] = next_states[:self.
                                                            max_capacity -
                                                            self.next_p]
            self.buf_done[self.next_p:self.
                          max_capacity] = dones[:self.max_capacity -
                                                self.next_p]
            self.if_full = True
    ...
    def sample_batch(self,
                     batch_size: int) -> Tuple(Tensor, Tensor, Tensor, Tensor):
        if self.if_use_per:
            start = -self.max_capacity
            end = (self.cur_capacity - self.max_capacity) if (
                self.cur_capacity < self.max_capacity) else None

            indices, is_weights = self.per_tree.get_indices_is_weights(
                batch_size, start, end)
            return (self.buf_state[indices], self.buf_action[indices],
                    self.buf_reward[indices], self.buf_next_state[indices],
                    self.buf_done[indices],
                    torch.as_tensor(
                        is_weights, dtype=torch.float32, device=self.device))

        indices = torch.randint(
            self.cur_capacity, size=(batch_size, ), device=self.device)
        '''replace indices using the latest sample'''
        i1 = self.next_p
        i0 = self.next_p - self.add_capacity
        num_new_indices = 1
        new_indices = torch.randint(
            i0, i1, size=(num_new_indices, )) % (
                self.max_capacity - 1)
...
class ReplayBuffer:  # for off-policy
    ...
    def __init__(self,
        ...
        self.buf_action = torch.empty((max_capacity, action_dim),
                                      dtype=torch.float32,
                                      device=self.device)
        self.buf_reward = torch.empty((max_capacity, 1),
                                      dtype=torch.float32,
                                      device=self.device)
        self.buf_mask = torch.empty((max_capacity, 1),
                                    dtype=torch.float32,
                                    device=self.device)

        buf_state_size = (max_capacity, state_dim) if isinstance(
            state_dim, int) else (max_capacity, *state_dim)
        self.buf_state = torch.empty(
            buf_state_size, dtype=torch.float32, device=self.device)
    ...
    def update_buffer(self, traj_list: List[List]):
        ...
        states, rewards, masks, actions = [
            torch.cat(item, dim=0) for item in traj_items
        ]
        self.add_capacity = rewards.shape[0]
        p = self.next_p + self.add_capacity  # update pointer
        ...
        if p > self.max_capacity:
            ...
            self.buf_reward[self.next_p:self.
                            max_capacity] = rewards[:self.max_capacity -
                                                    self.next_p]
            self.buf_mask[self.next_p:self.
                          max_capacity] = masks[:self.max_capacity -
                                                self.next_p]
            self.buf_action[self.next_p:self.
                            max_capacity] = actions[:self.max_capacity -
                                                    self.next_p]
    ...
    def sample_batch(self,
                     batch_size: int) -> Tuple(Tensor, Tensor, Tensor, Tensor):
        indices = torch.randint(
            self.cur_capacity - 1, size=(batch_size, ), device=self.device)
        '''replace indices using the latest sample'''
        i1 = self.next_p
        i0 = self.next_p - self.add_capacity
        num_new_indices = 1
    ...
    def sample_batch_per(
        ...
        return (
            ...
            torch.as_tensor(
                ...
                device=self.device)  # important sampling weights
        )
        ...
        if if_save:
            ...
            for obj, name in obj_names:
                if self.cur_capacity == self.next_p:
                    ...
                    buf_tensor = torch.vstack(
                        (obj[self.next_p:self.cur_capacity],
                         obj[0:self.next_p]))
    ...
    def get_state_norm(self,
                       cwd: str = '.',
                       state_avg: List[float, Tensor] = 0.0,
                       state_std: List[float, Tensor] = 1.0):
        try:
    ...
    def concatenate_buffer(self) -> Tuple(Tensor, Tensor, Tensor, Tensor):
        if self.prev_p <= self.next_p:
            ...
            buf_action = torch.vstack((
                self.buf_action[self.prev_p:],
                self.buf_action[:self.next_p],
            ))
            buf_reward = torch.vstack((
                self.buf_reward[self.prev_p:],
                self.buf_reward[:self.next_p],
            ))
            buf_mask = torch.vstack((
                self.buf_mask[self.prev_p:],
...
class ReplayBufferList(list):  # for on-policy
    ...
    def __init__(self):
        list.__init__(
            self
        )  # (buf_state, buf_reward, buf_mask, buf_action, buf_noise) = self[:]

    def update_buffer(self, traj_list: List[List]) -> Tuple(int, float):
    ...
    def get_state_norm(self,
                       ...
                       state_std: List[Tensor, float] = 1.0):
        try:
            torch.save(state_avg, f'{cwd}/env_state_avg.pt')
            torch.save(state_std, f'{cwd}/env_state_std.pt')
        except Exception as error:
...
class BinarySearchTree:
    ...
    def __init__(self, memo_len):
        ...
        self.prob_ary = np.zeros(
            (memo_len - 1) + memo_len)  # parent_nodes_num + leaf_nodes_num
    ...
    def update_ids(self, data_ids, prob=10):  # 10 is max_prob
        ...
        self.prob_ary[
            ids] = prob  # here, ids means the indices of given children (maybe the right ones or left ones)
        p_ids = (ids - 1) // 2
    ...
    def get_indices_is_weights(self, batch_size, start, end):
        ...
        is_weights = np.power(prob_ary,
                              -self.per_beta)  # important sampling weights
