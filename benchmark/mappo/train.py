...
from rltoolkit.policy.multiagent.mappo import MAPPO
...
def main():
    ...
    for agent_id in range(agent_num):
        share_observation_space = envs.share_observation_space[
            agent_id] if args.use_centralized_V else envs.observation_space[
                agent_id]
    ...
    for episode in range(episodes):
        for step in range(EPISODE_LENGTH):
            ...
            for agent_id in range(agent_num):
                ...
                buffers[agent_id].insert(share_obs,
                                         np.array(list(obs[:, agent_id])),
                                         actions[agent_id],
                                         action_log_probs[agent_id],
                                         values[agent_id],
                                         rewards[:, agent_id], masks[:,
                                                                     agent_id])
        ...
        train_infos = []
        for agent_id in range(agent_num):
            train_info = agents[agent_id].learn(buffers[agent_id], PPO_EPOCH,
                                                NUM_MINI_BATCH,
                                                args.use_popart)
        ...
        if episode % LOG_INTERVAL_EPISODES == 0:
            ...
            for agent_id in range(agent_num):
                for k, v in train_infos[agent_id].items():
                    ...
                    tensorboard.add_scalar(agent_k, v, total_num_steps)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--use_popart',
        default=True,
        help='whether to use PopArt to normalize rewards (default: True)')
    parser.add_argument(
        '--use_centralized_V',
        default=True,
        help='whether to use centralized V function(default: True)')
    parser.add_argument(
        '--use_value_active_masks',
        default=True,
        help='whether to mask useless data in value loss, (default: True)')
